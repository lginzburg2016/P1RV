...
void crea_porte(char tab[MSIZE][MSIZE], int numero){
	...
	//on choisit une case aleatoire qui n est ni dans la premiere ni dans la derniere colonne. Et qui est vide.
	...
	//on choisit deux cases pour creer la porte, en verifiant qu elles rentrent toutes les deux dans la matrice.
	int m1, n1, direction;
	while(crea_porte == false){
		//On choisit aleatoirement la direction (haut ou bas) dans laquelle on place la porte.
		direction = rand()%2;
		switch(direction){
		case 0:
			m1 = m+2;
		...
		if ((m1 > 0) && (m1 < MSIZE) && (n1 > 0) && (n1 < MSIZE) && ( (tab[min(m-1,m1-1)][n] == ' ') && (tab[max(m+1,m1+1)][n] == ' ') ) && ( (tab[m][n-1] == ' ') && (tab[m][n+1] == ' ') ) && ( (tab[m1][n-1] == ' ') && (tab[m1][n+1] == ' ') ) ){ //on ne veut pas des portes collees ou imbriquees !
			...
			tab[m][n] = 'X';
			tab[m1][n1] = 'X';
			//conversion de l entier numero en caractere
